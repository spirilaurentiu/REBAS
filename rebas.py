...
def plot1D(Y,
    ...
    # Plot lines, scatter points, or points with error bars.
    for ix, Y_series in enumerate(Y):
        if Yerr is None:
            if scatter:
            ...
            plt.errorbar(X[ix], Y_series, yerr=Yerr[ix], errorevery=Yerr_every,
                         fmt=marker, linestyle="None", alpha=alpha,
                         label=labels[ix], color=colors[ix] if colors else None)
        else:
            plt.errorbar(X[ix], Y_series, yerr=Yerr[ix], errorevery=Yerr_every,
...
def main(args):
    ...
    #region DRILL
    if DRILL_REQUIRED: # Files generated by VMD measure
        ...
        # Get names and assign print-ready names
        col_names_to_print = {}
        for header, _ in drill_data:
            for col_name in header:
                if col_name.startswith("Dist_DCD"):
                    col_names_to_print[col_name] = col_name
        #print("Column names to print:", col_names_to_print)
            ...
            for col_name, (hist, bin_edges) in hist_dict.items():
                ...
                if col_name[0:4] == "Dist":
                    ...
                    plot1D(
                        ...
                        title=f"Histogram of Bond Lengths",
                        xlabel="Distance (Å)",
                        ylabel=r"$\rho$",
                        labels=["300K" if col_name == "Dist_DCD1" else "600K"],
                        colors=["black"],
                        linestyle="-",
                        marker=None,
                        alpha=0.7
        ...
        for header, data in drill_data:
            ...
            for col_idx, col_name in enumerate(header):
                if col_name[0:4] == "Dist":
                    ...
                    stat, Prob = shapiro(col_data[::1000])  # Shapiro is sensitive to large datasets
    ...
    if OUTPUT_REQUIRED and TRAJECTORY_REQUIRED:
        ...
        if 'dpe' in args.checks:
            ...
            #plt.title("ΔE")
            plt.title("Thermodynamic States Work Distribution")
            plt.xlabel("W (kJ/mol)")
        ...
        if "rex_eff" in args.figures:
            ...
            for (sim_type, seed, replicaIx), subdf_group in out_df.groupby(["sim_type", "seed", "replicaIx"]):
                ...
                (ACF_rho, obs_tau, ess, t0_clean) = stats.autocorr2_revised(obs_data, lag_fraction=0.5, max_lag=50000)
                #print("obs_tau", obs_tau)
        ...
        if "traj_stats_0D" in args.figures:
            ...
            if DO_GEOMETRY:
                ...
                # =============================================================
                # OBSERVABLES averages Print and Plot
                # =============================================================
                ...
                for simIx in range(n_repeats):
                    ...
                    if PLOT__:
                        ...
                        Y_series = observables_avg[:, simIx, replicaIx, :]
                        YErr = observables_std[:, simIx, replicaIx, :]
                        ...
                        if args.useAgg:
                            ...
                            plt.show()
                # endregion # Observables average print and plot

                # =============================================================
                # OBSERVABLES std Print and Plot
                # =============================================================
                ...
                for simIx in range(n_repeats):
                    ...
                    if PLOT__:
                        ...
                        if args.useAgg:
                            ...
                            plt.show()
                # endregion # Observables std print and plot

                # =============================================================
                # OBSERVABLES std ratios Print and Plot
                # =============================================================
                # region Observable stdratio
                # Take the ratios of the standard deviations and scatter them
                numerator = observables_std[:, :, 1:, :]
                denominator = observables_std[:, :, :-1, :]
                print("numerator", numerator)
                print("denominator", denominator)
                obs_std_ratios = np.divide(
                    numerator,
                    denominator,
                ...
                if PLOT__:
                    ...
                    for replicaIx in range(n_thermos - 1):
                        ...
                        plot1D(
                            Y=[Y_series],
                            scatter=True,
                            title="Observable Std Ratios",
                            xlabel="Observable index",
                            ylabel="Std Ratio",
                            #labels=[f"{Ts[replicaIx + 1]:.1f} K / {Ts[replicaIx]:.1f} K"],
                            colors=[colorByReplica(replicaIx)],
                            instantiateFigure=False,
                            marker="o",
                        )

                        expected_ratio = np.sqrt(Ts[replicaIx + 1] / Ts[replicaIx])
                        plt.axhline(1.0 / expected_ratio, color="black", linestyle="--")
                        plt.axhline(expected_ratio, color="black", linestyle="--")
                        plt.axhline(1, color="gray", linestyle="--")

                    if args.useAgg:
                        plt.savefig("obs_std_ratios.png")
                    else:
                        plt.show()
            #endregion traj_std_ratios
        ...
        if "traj_stats_2D" in args.figures:
            ...
            # Prepare constants for 2D histogram calculations
            kB = 0.008314462618
            temperatures = {
            ...
            for typeIx in range(len(uniq_types1)):
                for repeatIx in range(len(uniq_repeats1)):
                    for thermoIx in range(len(uniq_thermos1)):
                        ...
                        hists_2D[typeIx, repeatIx, thermoIx, :, :] = hist2d

                        total = np.sum(hist2d)
                        if total == 0.0:
                            continue

                        # Get probabilities and PMFs
                        Prob = hist2d / total
                        pmf = np.full_like(Prob, np.nan, dtype=float)
                        mask = Prob > 0.0
                        pmf[mask] = -kB * temperatures[args.moleculeName][thermoIx] * np.log(Prob[mask])
                        pmf -= np.nanmin(pmf)
                        PMFs_2D[typeIx, repeatIx, thermoIx, :, :] = pmf

            # Calculate basin-based transition probabilities and MFPTs (ala1 phi/psi basins only)
            if args.moleculeName == "ala1":
                basin_names = list(Observables.ALA1_BASINS.keys())  # C5, PPII, C7_eq, alpha_eq
                print("Basins:", basin_names)
                n_basins = len(basin_names)
                basin_lag = 1  # lag in frames used for basin transition counting

                def classify_basin(phi_rad, psi_rad):
                    """Assign each frame to a basin index (radians), or -1 if unclassified."""
                    labels = np.full(phi_rad.shape, -1, dtype=int)
                    for bIx, bname in enumerate(basin_names):
                        b = Observables.ALA1_BASINS[bname]
                        mask = (
                            (labels == -1)
                            & (phi_rad >= b["phi_min"]) & (phi_rad <= b["phi_max"])
                            & (psi_rad >= b["psi_min"]) & (psi_rad <= b["psi_max"])
                        )
                        labels[mask] = bIx
                    return labels
                ...
                for typeIx in range(len(uniq_types1)):
                    for repeatIx in range(len(uniq_repeats1)):
                        for thermoIx in range(len(uniq_thermos1)):
                            ...
                            labels = classify_basin(phi, psi)

                            counts = np.zeros((n_basins, n_basins), dtype=float)
                            for frameIx in range(len(labels) - basin_lag):
                                a, b = labels[frameIx], labels[frameIx + basin_lag]
                                if a == -1 or b == -1:
                                    continue
                                counts[a, b] += 1.0
                            ...
                            T_basins = counts / row_sums

                            T_basin_storage[typeIx, repeatIx, thermoIx, :, :] = T_basins

                            # 1. Compute eigenvalues and eigenvectors of T.T
                            eigvals, eigvecs = np.linalg.eig(T_basins.T)

                            # 2. Find the index of the eigenvalue closest to 1.0
                            idx = np.argmin(np.abs(eigvals - 1.0))

                            # 3. Extract the 1D eigenvector, take the real part, and clean numerical noise
                            stat_dist = np.real(eigvecs[:, idx])
                            stat_dist = np.maximum(stat_dist, 0.0)  # eliminate negative floating-point artifacts

                            # Handle the case where the solver picked the opposite sign
                            if stat_dist.sum() == 0:
                                stat_dist = np.maximum(-np.real(eigvecs[:, idx]), 0.0)

                            # 4. Normalize to sum to 1
                            stat_dist = stat_dist / stat_dist.sum()

                            print("pi:", end=' ')
                            for val in stat_dist:
                                print(val, end=' ')
                            print()
                ...
                for typeIx in range(len(uniq_types1)):
                    for thermoIx in range(len(uniq_thermos1)):
                        ...
                        #row_sums[row_sums == 0.0] = 1.0
                        #T_avg_byRep[typeIx, thermoIx, :, :] /= row_sums
            ...
            for typeIx in range(len(uniq_types1)):
                for repeatIx in [0]: # range(len(uniq_repeats1)):
                    for thermoIx in [0]: # range(len(uniq_thermos1)):
                        plt.figure(figsize=(8, 6))
                        mesh = plt.pcolormesh(x_edges, y_edges, PMFs_2D[typeIx, repeatIx, thermoIx, :, :].T, shading='auto', cmap='bwr')
